...
for(i in 1:ncol(mycombn)){
	...
	mytb <- differential.tests(MATRIX=dat[, ind], GROUPS=GROUPS[ind], GROUP1=g1, GROUP2=g2, TEST="wilcox.test",
						PAIRED=mypaired, CUTOFF=0.1, COLNAME="feature", FILE=paste("wilcox.test", tag, study, sep="."), DESCRIPTION=DESCRIPTION)

	myfeatures <- as.character(mytb[which(mytb$p.value<=0.1), "feature"])
