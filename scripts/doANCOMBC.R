...
if(doFilt==TRUE){
	...
	write.table(mydata, file=paste("filtered.rel.freqs",tail(unlist(strsplit(unlist(strsplit(counts.file,split = ".tsv")[1]),split = "/")),1), sep="."), sep="\t", row.names=FALSE, quote=FALSE)
...
out <- ancombc(phyloseq=physeq, formula=myformula, 
              p_adj_method="fdr", pvr_cut=0.95, lib_cut=0, 
              group="group", struc_zero=structzero, neg_lb=FALSE, tol=1e-5, 
              max_iter=100, conserve=TRUE, global=FALSE)
res <- out$res
...
# filtramos despues de transformar
...
#  necesario eliminar ../ esta es la de impresion pdf
mytb <- differential.tests(MATRIX=dat.pct[, ind], GROUPS=GROUPS[ind], GROUP1=g1, GROUP2=g2, TEST="wilcox.test",
						PAIRED=mypaired, CUTOFF=0.1, COLNAME="feature", FILE=paste("ANCOMBC.test", tag, study, sep="."), DESCRIPTION=DESCRIPTION, mypdf=FALSE)
...
items.group2 <- which(is.element(colnames(dat.trans), G2))
 # revisar si tag y study tiene ../ si lo tienen habra que modificar 
...
write.table(dat.trans, file=paste("ANCOMBC.normalized", counts.file, sep="."), sep="\t", row.names=FALSE, quote=FALSE)write.table(dat.trans, file=paste("ANCOMBC.normalized", counts.file, sep="."), sep="\t", row.names=FALSE, quote=FALSE)
# wrong?
# write.table(dat.trans, file=paste(tail(unlist(strsplit(unlist(strsplit(counts.file,split = ".tsv")[1]),split = "/")),1),"ANCOMBC.normalized", sep="."), sep="\t", row.names=FALSE, quote=FALSE)
#    habra que modificar ../
