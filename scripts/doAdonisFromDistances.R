...
if ("myfunctions.downstream.R" %in% list.dirs()){
  ...
  source("C:/Users/RositaRos/Desktop/analysis/estudio/scripts/myfunctions.downstream.R")
}
...
adonis.res <- adonis2(myD ~ GROUPS, data=mydf, perm=600)

save(adonis.res, file=paste(study, "RData", sep="."))

adonis.p.value <- format(adonis.res["GROUPS", "Pr(>F)"], digits=3)

adonis.R2 <- format(adonis.res["GROUPS", "R2"], digits=3)

# Para el adonis analysis 
# if(mylevels==""){
